...
class AdjustGpsAltitude(next: LocationStackItem, storage: StorageInterface) :  LocationStackChainedItem(next) {
    private val saltitude: SolidProvideAltitude
    private val senabled: SolidAdjustGpsAltitude
    private val sadjust: SolidAdjustGpsAltitudeValue
    private var adjust: Int
    private var enabled: Boolean
    private val altitude = AltitudeCache()

    init {
        senabled = SolidAdjustGpsAltitude(storage)
        sadjust = SolidAdjustGpsAltitudeValue(storage)
        saltitude = SolidProvideAltitude(storage, SolidUnit.SI)
