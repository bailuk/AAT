...
class RoutePainterNode(private val mcontext: MapContext, private val color: Int, minPixelSpace: Int)
    ...
    override fun drawNode(node: TwoNodes.PixelNode) {
        ...
        val altitudeColor = if (altitude == ElevationProvider.NULL_ALTITUDE) {
            color
        } else {
            AltitudeColorTable.getColor(altitude.toInt())
        }
        mcontext.draw().bitmap(mcontext.draw().getNodeBitmap(), node.pixel, altitudeColor)
    }
}
