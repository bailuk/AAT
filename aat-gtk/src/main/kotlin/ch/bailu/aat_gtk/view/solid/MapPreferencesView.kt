...
class MapPreferencesView (storage: StorageInterface, actionHelper: ActionHelper, window: Window) : VerticalScrollView() {
    init {
        add(Res.str().p_map())
        ...
        add(SolidDirectorySelectorView(GtkSolidTileCacheDirectory(storage, GtkAppConfig.Foc) , actionHelper, window).layout)
        add(SolidBooleanSwitchView(SolidVolumeKeys(storage)).layout)

        add(Res.str().p_offline_map())
        add(SolidDirectorySelectorView(solidMapDirectory, actionHelper, window).layout)
        add(SolidDirectorySelectorView(SolidMapsForgeMapFile(storage, GtkAppConfig.Foc, GtkMapDirectories(storage, GtkAppConfig.Foc)), actionHelper, window).layout)
        add(SolidDirectorySelectorView(SolidRenderTheme(solidMapDirectory, GtkAppConfig.Foc), actionHelper, window).layout)
        add(SolidBooleanSwitchView(MapsForge(storage)).layout)

        add(ToDo.translate("Dem3 altitude tiles"))
        add(SolidDirectorySelectorView(GtkSolidDem3Directory(storage, GtkAppConfig.Foc), actionHelper, window).layout)
