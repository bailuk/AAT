...
class MainStackView (actionHelper: ActionHelper, services: ServicesInterface, storage: StorageInterface, dispatcher: DispatcherInterface, window: Window) {

    val layout = Stack()

    private val preferences = PreferencesStackView(storage, window)
    private val map = MapMainView(actionHelper, services, storage, dispatcher )
