...
cd ../../ || exit 1
./gradlew aat-awt::build || exit 1

ssh $remote "test -d ${data} || mkdir ${data}" || exit 1
scp  aat-awt/build/libs/aat-awt-all.jar "${remote}:${data}/aat.jar"  || exit 1
scp  aat-awt/src/main/resources/images/icon.svg "${remote}:${data}/aat.svg" || exit 1
